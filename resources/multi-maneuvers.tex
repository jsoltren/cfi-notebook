...
\emph{Memory aid: Gear UP - 12" MP. Gear DOWN - 15" MP.}
...
Pitch controls airspeed (trim full aft!) and power controls altitude (approximately \underline{\textbf{18''-19''}}).
...
In the Duchess we fly steep turns using the trim method.

Roll into a \textbf{50\degree{} bank}, +/- 5\degree{}, using the following method:

Smoothly roll in to a level turn with 50\degree{} bank to the left over the course of 2-3 seconds.
Do not apply yoke forward or back pressure. Upon crossing through
30\degree{} of bank, smoothly turn the trim wheel one full turn for nose up trim. At 50\degree{} of bank,
be patient for 2-3 seconds, then apply another turn of trim. On 33G, align the screw directly in
front of either the pilot or copilot seat on the horizon. Maintain altitude primarily using bank:
climbing? Roll in to 55\degree{}. Descending? Roll out to 45\degree{}.

Transitioning from left to right, apply forward pressure so as to not balloon. Remove the forward
pressure upon passing through 30\degree{} of bank in the other direction. Some pilots tend to bank one way
more than the other, especially right seat pilots dealing with parallax on the attitude indictaor.
So, it may be necessary to apply an additional turn of the trim wheel.

Rudder for coordination: left rudder to the left, right rudder to the right.

When correctly configured, the airplane will be able to maintain a steep turn with little to no control inputs.

Maintain altitude and roll out on chosen landmark.
...
Leave the throttles at 11'' MP to avoid shock cooling.

Smoothly pitch nose \textbf{20\degree{}} down.

Smoothly roll into \textbf{30\degree{}} - 45\degree{} bank.

\textbf{Do not exceed 140 KIAS with gear down.}

Recovery:
\begin{itemize}[label={}]
\item Smoothly roll \& pitch to straight \& level (lead level off by 200’).
\item \textbf{100 kts} reduce power to \textbf{20''/2400}
\item Verify below gear retraction speed.
\item Perform post-maneuver GUMP-CS
\end{itemize}
...
Right throttle \textbf{12''} / Right propeller \textbf{detent}.
