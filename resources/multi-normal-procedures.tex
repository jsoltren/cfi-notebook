...
\subsection{Short-Field Takeoff}

For safety reasons we perform our short field takeoff at a speed closer to \vyse. Any time you have wheels off the
ground below \vyse, we risk a \vmc roll.

\begin{itemize}[label={}]
    \item Use all available runway.
    \item Hold brakes.
    \item Throttles to 20" MP.
    \item Continue to hold brakes, full throttle. Release brakes if we are unable to stay stopped (wet runway?).
    \item Rotate at 80 KIAS
    \item Gear up (when positive rate of climb)
    \item Climb at \vxse which in this airplane happens to equal \vyse.
    \item Transition to 90 KIAS when clear of obstacles.
\end{itemize}
...
Stay off the brakes until at or below 45 knots (ideally 40).

\subsection{Short-Field Approach and Landing}

All available drag: full flaps, props forward.

On airspeed, on altitude, on glide slope. Pull the power over the runway threshold, or later if substantial winds.

On the 1000 foot marks, +100/-0. But don't force it down. Go around is preferable.

Yoke to the lap.

Simulated Max Braking.

DO NOT touch flap lever until clear of the runway!
...
FINAL    & \textbf{\textcolor{blue}{85}}    & \textbf{Reduce/2600}      & \textbf{20\degree{} (for training)}
\end{tabular}
\end{table}
